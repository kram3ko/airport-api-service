...
class TicketCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not flight:
            raise ValidationError(f"No flight found from {source.name} to {destination.name}")
        ...
        if not has_seat and not has_rows_seats:
            raise ValidationError("You must specify either seat or a combination of rows and seat_numbers")

        if has_seat and has_rows_seats:
            raise ValidationError("You can only specify one option: seat or rows+seat_numbers")
        ...
        if has_rows_seats:
            try:
                ...
                raise ValidationError("Rows and seat numbers must be integers separated by commas")

            if len(rows) != len(seat_numbers):
                raise ValidationError("The number of rows must match the number of seat numbers")
...
class RouteBasedTicketBookingSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if data.get('source') == data.get('destination'):
            raise serializers.ValidationError("Departure and arrival airports must differ")
        ...
        if not flight:
            raise serializers.ValidationError("No matching flights found for the given route")
        ...
        if 'rows' in data and 'seat_numbers' in data:
            ...
            if len(rows) != len(seat_numbers):
                raise serializers.ValidationError("The number of rows and seat numbers must match")
        ...
        for seat_data in data['seats']:
            ...
            if row not in available_rows:
                raise serializers.ValidationError(f"Row {row} does not exist on the airplane")
            if seat not in available_seats_in_row:
                raise serializers.ValidationError(f"Seat {seat} does not exist in row {row}")

            # Check if the seat is already booked
            if (row, seat) in booked_seats:
                raise serializers.ValidationError(f"Seat {row}-{seat} is already booked")
